...
A Railway **project** access token, used only when `PRIMO_DOMAIN_PROVIDER` is
`railway`. Create it in the Railway dashboard under the project's Settings →
Tokens (project-scoped, least privilege). The project/environment/service IDs
Railway injects automatically (`RAILWAY_PROJECT_ID`, etc.) supply the rest.
